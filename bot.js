...
client.on('guildMemberAdd', member => {
     const welcomer =  member.guild.channels.find('name', 'road');
    if(!welcomer) return;
      if(welcomer) {
         moment.locale('ar-ly');
         var m = member.user;
...
welcomer.sendFile(canvas.toBuffer())
 ...
 client.on("message", message => {
	    ...
	    let channel = message.guild.channels.find("name", "sumbite")
            if(!channel) return message.reply("**لانشاء روم التقديمات #setsumbite من فضلك اكتب الامر**")
            if(channel) {
              ...
              m.channel.awaitMessages( m1 => m1.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m1) => {
                  ...
                  m.edit(message.member + ', **:timer:**').then( (m) =>{
                      ...
                      m.channel.awaitMessages( m2 => m2.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m2) => {
                          ...
                          m2.delete()
                          message.channel.send( message.member + ', **:timer:**').then( (m) =>{
                            m.edit( message.member + ', **هل بتتفاعل**' )
                            setTimeout(() => {
                              m.delete()
                            }, 10000);
                            m.channel.awaitMessages( m1 => m1.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m3) => {
                                ...
                                m3.delete();
                                message.channel.send( message.member + ', **:timer:**').then( (m) =>{
                                  m.edit( message.member + ', **ليش تبي الرتبه**' )
                                  setTimeout(() => {
                                    m.delete()
                                  }, 10000);
                                  m.channel.awaitMessages( m1 => m1.author == message.author,{ maxMatches: 1, time: 60*1000 } ).then ( (m4) => {
                                      ...
                                      m4.delete();
                                      message.channel.send( message.member + ', **:timer:**').then( (m) =>{
                                        m.edit( message.member + ', **هل بتضع الشعار**' )
                      ...
                      m.edit(message.member + ', **جارى جمع البيانات**').then( (mtime)=>{
                        setTimeout(() => {
                          let embed = new Discord.RichEmbed()
                        .setColor('RANDOM')
                        .setTitle(`تقديم سبورت في ${message.guild.name}`)
                        .addField('**الاسم**', `${name}` , true)
                        .addField('**العمر**', `${age}` , true)
                        .addField('**هل بتتفاعل**',`${ask}`)
                        .addField('**ليش تبي الرتبه**',`${ask2}`)
                        .addField('**هل بتضع الشعار**',`${ask3}`)
        ...
        client.on('message', message=>{
            if(message.content.startsWith(prefix+"setsumbite")) {
		    if(!message.channel.guild) return;
                if(message.author.bot) return;
                if(!message.member.hasPermission('ADMINISTRATOR')) return message.reply("**تحتاج الى `MANAGE_CHANNELS`**");
                message.guild.createChannel("sumbite", "text").then(c =>{
  ...
  if(message.content.startsWith(prefix + "accept")) {
    let acRoom = message.guild.channels.find('name', 'apply-refuse');
    if(!acRoom) return message.reply("!!setac من فضلك انشاء روم **•-apply** او اكتب الامر");
    if(acRoom) {
    if(!message.guild.member(message.author).hasPermission("MANAGE_ROLES")) return;
    if(!mention) return message.reply('منشن شخص');
    if(!role) return message.reply('« Support');
...
client.on('message',async message => {
  ...
  if(!message.channel.guild) return;
  let acRoom = message.guild.channels.find('name', 'apply-refuse');
  if(!acRoom) return message.reply("#setapply من فضلك انشاء روم **•-apply** او اكتب الامر");
  if(!message.guild.member(message.author).hasPermission("MANAGE_ROLES")) return;
  if(!mention) return message.reply("منشن شخص");
  ...
  acRoom.send(`**${mention} تم رفضك للاسف**`)
		 ...
		 if(!message.channel.guild) return;
                if(message.author.bot) return;
                if(!message.member.hasPermission('ADMINISTRATOR')) return message.reply("**تحتاج الى `MANAGE_CHANNELS`**");
                message.guild.createChannel("apply-refuse", "text").then(c =>{
                    c.overwritePermissions(message.guild.id, {
                        SEND_MESSAGES: false
